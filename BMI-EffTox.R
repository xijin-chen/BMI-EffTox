...
sim_Aug <- function(decision_t = 20, num_doses = 5, eff_t = 12, tox_t = 4,
                    a = 0, b = 0.5, c_d = c(4.427152, 4.269323, 4.140692, 4.071614),
                    threshold = 20, sd = 2,  zeta_mean = 2,
                    Lp=2.07, p1e=0.4, p2t=0.7, tau=0.5,
                    ...
                    start_dose = 1, num_sims=5000, # cohorts=10, 
                    cohort_size = 3, chains=4){
  
  out <- foreach(sim = 1:num_sims, .export = "model_efftox_xc", .packages="rstan") %dorng% {
    # -----------------------------------------------------------
    stop <- decision_t + 4
    cohorts <- seq(4, stop, by=4) # recruit a cohort every 4 weeks
    # -----------------------------------------------------------
    ctDNA_M = array(NA, dim = c(length(cohorts), eff_t, cohort_size))
    ctDNA_num = array(NA, dim = c(length(cohorts), stop, cohort_size))
    C_max = array(NA, dim = c(length(cohorts), length(cohorts), cohort_size))
    ...
    patient_spc_RV = matrix(NA, ncol=cohort_size, nrow=length(cohorts))
    
    eff_outcome_list <- array(NA)
    ctDNA_num_list <- array(NA)
    C_max_list <- array(NA)
    ctDNA_M_list <- array(NA)
    ...
    mod_ptox_mean <- matrix(NA, nrow=length(cohorts), ncol=num_doses)
    mod_ptox_se <- matrix(NA, nrow=length(cohorts), ncol=num_doses)
    mod_peff_mean <- matrix(NA, nrow=length(cohorts), ncol=num_doses)
    mod_peff_se <- matrix(NA, nrow=length(cohorts), ncol=num_doses)
    mod_utility_mean <- matrix(NA, nrow=length(cohorts), ncol=num_doses)
    mod_utility_se <- matrix(NA, nrow=length(cohorts), ncol=num_doses)
    ...
    for(i in cohorts){  
      ...
      patient_spc_RV[cohort_num, 1:cohort_size] <- rnorm(cohort_size, mean = 0, sd=tau)
      
      for(patient in 1:cohort_num){
        ctDNA_num[patient, (arrived_t[patient]+1):i, ] <- (1:(i-arrived_t[patient]))
      }
      ctDNA_num[which(ctDNA_num >= (eff_t))] <- (eff_t)
      
      
      for(cohort in 1:cohort_size){
        # --------------------- generate ctDNA values -------------
        for(current_t in 1:eff_t){
          ctDNA_M[cohort_num, current_t, cohort] <- 
            rnorm(n = 1, 
                  mean = c_d[administerdose[cohort_num]] + 
                    b*current_t + a*current_t^2,
                  sd = sd) + patient_spc_RV[cohort_num, cohort]
        }
        # --------------------- generate ctDNA values -------------
        for(j in 1:cohort_num){
          if(ctDNA_num[j, cohort_num*4, cohort] < (eff_t-tox_t+4)){
            C_max[j,cohort_num,cohort] <- 
              max(ctDNA_M[j, ctDNA_num[j, ((arrived_t[j]+1):(cohort_num*4)), cohort], cohort])
            eff_outcome[j,cohort_num,cohort] <-
              ifelse(C_max[j,cohort_num,cohort] >= threshold, 0, 2)
            eff_miss_outcome[j,cohort_num,cohort] <- 
              ifelse(C_max[j,cohort_num,cohort] >= threshold, 0, 1)
          }else{
            C_max[j,cohort_num,cohort] <- 
              max(ctDNA_M[j, ctDNA_num[j,((arrived_t[j]+1):(cohort_num*4)),cohort], cohort])
            eff_outcome[j,cohort_num,cohort] <-
      ...
      utility_mean <- matrix(NA, nrow = 1, ncol=num_doses) # mean value of utility
      for (num_d in 1:num_doses){
        ...
        mod_ptox_mean[cohort_num,num_d] <- sum_list[exps_toxicity,'mean']
        mod_ptox_se[cohort_num,num_d] <- sum_list[exps_toxicity,'se_mean']
        mod_peff_mean[cohort_num,num_d] <- sum_list[exps_efficacy,'mean']
        mod_peff_se[cohort_num,num_d] <- sum_list[exps_efficacy,'se_mean']
        mod_utility_mean[cohort_num,num_d] <- sum_list[exps_utility,'mean']
        mod_utility_se[cohort_num,num_d] <- sum_list[exps_utility,'se_mean']
      }
      # ----- dose admissibility -------
      ...
      if(sum(acceptable)){
        ...
        administerdose_current <- acceptable_doses[which.max(mod_utility_mean[cohort_num,acceptable_doses])]
    ...
    # ------------------------------- output format --------------------------------
    tox_outcome_output <- c(t(tox_outcome))
    eff_outcome <- replace(eff_outcome, eff_outcome==2, '-')
    ctDNA_M <- round(ctDNA_M, 3)
    ...
    iter_out <- list('recommended_dose' = recommenddose,
                     ...
                     'ctDNA numbers' = ctDNA_num,
                     'ctDNA_matrix' =  ctDNA_M,
                     'ctDNA_maximum' = C_max,
                     'mod_ptox_mean' = mod_ptox_mean,
                     'mod_ptox_se' = mod_ptox_se,
                     'mod_peff_mean' = mod_peff_mean,
                     'mod_peff_se' = mod_peff_se,
                     'mod_utility_mean' = mod_utility_mean,
                     'mod_utility_se' = mod_utility_se,
